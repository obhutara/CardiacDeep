...
@ex.automain
def main():
    label = np.load("C:\\Users\\omkbh\\Downloads\\bcnn-master\\bcnn-master\\predict\\test_targets.npy")[28]
    #pred = np.load("C:\\Users\\omkbh\\Downloads\\bcnn-master\\bcnn-master\\predict\\dropout\\dropout_pred.npy")[324]
    #std = np.load("C:\\Users\\omkbh\\Downloads\\bcnn-master\\bcnn-master\\predict\\dropout\\dropout_sigmoids.npy")[324]

    #val = std.mean()# + (std.std())
    #calculate_pavpu(pred, label, std, accuracy_threshold=0.78, uncertainty_threshold=val)
    #print()

    pred = np.load("C:\\Users\\omkbh\\Downloads\\bcnn-master\\bcnn-master\\predict\\bayesian\\bayesian_pred.npy")[28]
    std = np.load("C:\\Users\\omkbh\\Downloads\\bcnn-master\\bcnn-master\\predict\\bayesian\\bayesian_sigmoids.npy")[28]
