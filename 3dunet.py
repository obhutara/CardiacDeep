...
seed = 43
training_split_ratio = 0.90  # use 90% of samples for training, 10% for testing
num_epochs = 10
...
dice_score, dice_losses = [], []

# Outputs nifti files for each validation sample
for i in range(len(validation_loader)):
    batch = next(iter(validation_loader))
    model.eval()
    inputs, targets = prepare_batch(batch,training_batch_size, device) #2nd argument: training_batch_size or 16
    with torch.no_grad():
        logits = forward(model, inputs)
    ...
    dice_losses.append(get_dice_loss(F.softmax(logits,dim=CHANNELS_DIMENSION),targets))
